...
# 로그인 시 입력받은 S3 인증 URL을 저장할 변수 (세션을 사용하므로 사실상 백업용)
S3_AUTH_KEY = None
...
def login():
    ...
    if request.method == 'POST':
        ...
        if auth_url and auth_url.startswith('http'):
            ...
            # 로그인 성공 시, 메인 대시보드 페이지('/')로 리디렉션
            return redirect(url_for('index'))
        else:
            error = '유효한 URL 형식을 입력해주세요 (예: http://...)'
            
    # GET 요청이거나 로그인 실패 시 로그인 페이지를 보여줌
    return render_template('login.html', error=error)

@app.route("/")
def index():
    """메인 대시보드 페이지"""
    # 세션에 로그인 정보가 있는지 먼저 확인
    if not session.get('logged_in'):
        # 로그인 정보가 없으면 /login 페이지로 강제 이동
        return redirect(url_for('login'))
    
    # 로그인 정보가 있으면 메인 대시보드 페이지를 보여줌
    return render_template("LCD_v2.html")

@app.route("/video_feed")
def video_feed():
    """MJPEG 비디오 스트림을 제공"""
    # 비디오 스트림 접근 시에도 로그인 여부 확인
    if not session.get('logged_in'):
        return "Unauthorized", 401 # 권한 없음 응답
