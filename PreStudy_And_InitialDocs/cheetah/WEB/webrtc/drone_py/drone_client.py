# drone_client.py (InvalidStateError 및 AttributeError 최종 해결 버전)
...
def parse_candidate(candidate_str):
    ...
    return {
        ...
        'protocol': parts[2].lower(), 'priority': int(parts[3]),
...
class YoloVideoStreamTrack(VideoStreamTrack):
    ...
    def __init__(self, model_name, cam_num, confidence_threshold):
        super().__init__()
        self.confidence_threshold, self.frame_count, self._timestamp = confidence_threshold, 0, 0
        logging.info("YOLO 모델 로딩...")
        try:
            self.model = YOLO(model_name)
            self.model.conf, self.model.iou = confidence_threshold, 0.45
            logging.info(f"YOLO 모델 로딩 완료: {model_name}")
        except Exception as e: logging.error(f"YOLO 모델 로딩 실패: {e}"); raise
        logging.info("웹캠 초기화...")
        try:
            self.cap = cv2.VideoCapture(cam_num, cv2.CAP_DSHOW)
            if not self.cap.isOpened(): raise IOError(f"웹캠 {cam_num}을 열 수 없습니다.")
            self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640); self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
            self.cap.set(cv2.CAP_PROP_FPS, 30); self.cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
            logging.info("웹캠 초기화 완료.")
        except Exception as e: logging.error(f"웹캠 초기화 실패: {e}"); raise

    async def recv(self):
        pts, time_base = await self.next_timestamp()
        try:
            success, frame = await asyncio.to_thread(self.cap.read)
            if not success or frame is None: return self._create_black_frame(pts, time_base)
            self.frame_count += 1
            results = await asyncio.to_thread(self.model, frame, verbose=False)
            annotated_frame = results[0].plot()
            if self.frame_count % 90 == 0: self._log_detections(results)
            new_frame = av.VideoFrame.from_ndarray(annotated_frame, format="bgr24")
            new_frame.pts, new_frame.time_base = pts, time_base
            return new_frame
    ...
    async def next_timestamp(self):
        if hasattr(self, 'stopped') and self.stopped: pts = self._timestamp
        else:
            now = time.time()
            if not hasattr(self, "_start_time"): self._start_time = now
    ...
    def _create_black_frame(self, pts, time_base):
        ...
        cv2.putText(img, "Stream Error", (50, 240), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    ...
    def _log_detections(self, results):
        ...
        if detections: logging.info(f"🎯 감지된 객체 ({len(detections)}개): {', '.join([self.model.names[int(cls)] for cls in detections.cls])}")
        else: logging.info("📹 프레임 처리 중... (객체 미감지)")

    def stop(self):
        if not hasattr(self, 'stopped') or not self.stopped:
            self.stopped = True
            if hasattr(self, 'cap') and self.cap.isOpened(): self.cap.release(); logging.info("웹캠 리소스를 해제했습니다.")
            super().stop()
...
async def run():
    ...
    try:
        ...
        await websocket.send(json.dumps({"type": "offer", "offer": {"type": offer.type, "sdp": offer.sdp}}))
        ...
        async for message in websocket:
            ...
            if msg_type == "answer":
                ...
                # [수정 1] 'answer'를 처리하기 전에 현재 상태를 확인하는 방어 코드 추가
                if pc.signalingState == "have-local-offer":
                    ...
                    logging.warning(f"'answer'를 처리할 수 없는 상태입니다(현재 상태: {pc.signalingState}). 메시지를 무시합니다.")
                ...
                if candidate_data and candidate_data.get("candidate"):
                    ...
                    if parsed_info:
                        full_candidate_info = {**parsed_info, "sdpMid": candidate_data.get("sdpMid"), "sdpMLineIndex": candidate_data.get("sdpMLineIndex")}
                        try:
                        ...
                        except Exception as e: logging.warning(f"파싱된 Candidate 추가 실패: {e}")
                    else: logging.warning(f"Candidate 문자열 파싱 실패: {candidate_data['candidate']}")
        ...
        if 'pc' in locals() and pc.connectionState != "closed": await pc.close(); logging.info("🔌 PeerConnection을 종료했습니다.")
        # [수정 2] 웹소켓 상태 확인을 .closed로 변경
        if 'websocket' in locals() and not websocket.closed: await websocket.close(); logging.info("🔌 시그널링 연결을 종료했습니다.")
