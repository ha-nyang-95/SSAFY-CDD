...
class S3Manager:
    def __init__(self):
        ...
        try:
            ...
            logging.error(f"S3 클라이언트 초기화 또는 버킷 연결에 실패했습니다: {e}")
            send_log_to_loki({
                "event_type": "s3_connection_failure",
                "error": str(e)
            }, job_name="s3-operations")
    ...
    def upload_fileobj(self, file_obj, s3_object_key: str, content_type: str = 'application/octet-stream'):
        ...
        try:
            ...
            logging.info(f"파일 객체를 '{self.bucket_name}' 버킷에 '{s3_object_key}'로 성공적으로 업로드했습니다.")
        except ClientError as e:
            logging.error(f"S3 업로드 중 오류가 발생했습니다: {e}")
            send_log_to_loki({
    ...
    def download_fileobj(self, s3_object_key: str, file_obj):
        ...
        try:
            ...
            logging.info(f"'{self.bucket_name}' 버킷의 '{s3_object_key}' 객체를 파일 객체로 성공적으로 다운로드했습니다.")
            file_obj.seek(0)
        except ClientError as e:
            logging.error(f"S3 다운로드 중 오류가 발생했습니다: {e}")
            send_log_to_loki({
                "event_type": "s3_download_failure",
                "bucket": self.bucket_name,
                "key": s3_object_key,
                "error": str(e)
            }, job_name="s3-operations")
