...
CREATE TABLE `users` (
    ...
    `password`       VARCHAR(128) NOT NULL COMMENT 'Bcrypt 해시 처리',
    `name`           VARCHAR(20)  NOT NULL,
    `region`         VARCHAR(20)  NOT NULL COMMENT 'Region 이넘 문자열(예: SEOUL, GYEONGGI 등)',
...
CREATE TABLE `districts` (
    ...
    `region`      VARCHAR(20)  NOT NULL COMMENT 'Region 이넘 문자열(예: SEOUL, GYEONGGI 등)',
    `name`        VARCHAR(50)  NOT NULL COMMENT '예: 강남구 / 성남시 분당구',
    PRIMARY KEY (`district_id`),
    UNIQUE KEY `uk_district_region_name` (`region`, `name`)
);
...
CREATE TABLE `tasks` (
    ...
    `deleted_at`     TIMESTAMP NULL,
    PRIMARY KEY (`task_id`),
    UNIQUE KEY `uk_tasks_s3_name` (`s3_name`),
...
CREATE TABLE `cracks` (
    ...
    `task_id`         BIGINT      NOT NULL,
    `crack_id_string` VARCHAR(50) NOT NULL COMMENT 'crackId0, crackId1 등의 문자열',
    `status`          ENUM ('ACTIVE', 'INACTIVE', 'DELETED') NOT NULL DEFAULT 'ACTIVE',
    `activated_at`    TIMESTAMP   NOT NULL DEFAULT NOW(),
    `deactivated_at`  TIMESTAMP NULL,
