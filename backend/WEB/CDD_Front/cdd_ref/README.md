...
```bash
npm install react react-dom next styled-components axios @tanstack/react-query && npm install -D @storybook/react-webpack5 @storybook/addon-essentials @storybook/addon-interactions @storybook/addon-links @storybook/blocks @storybook/testing-library eslint prettier husky typescript @types/react @types/node @types/styled-components
```
...
*   **그림자 (Box Shadow):**
    *   `- [ ] `shadow-md`: `0 10px 30px rgba(0,0,0,0.1)` (modal content, auth box)`

#### **1.2. 재사용 가능한 컴포넌트 (Atomic Design)**

*   **Atoms (원자):**
    *   `- [ ] Button: `variant` (primary, secondary, danger, nav, auth, switch-auth), `size` (sm, md, lg), `onClick`, `disabled`, `children`
    *   `- [ ] Input: `type` (text, password), `placeholder`, `value`, `onChange`, `readOnly`, `required`
    *   `- [ ] Label: `htmlFor`, `children`
    *   `- [ ] Icon: `name` (video, 3d-model, structure), `size`, `color`
    *   `- [ ] Text: `variant` (h1, h2, h3, p, span, strong), `color`, `fontSize`, `fontWeight`, `children`
    *   `- [ ] Badge: `variant` (high, medium, low), `children`
*   **Molecules (분자):**
    *   `- [ ] UserProfileDisplay: `username`, `isLoggedIn`, `onLogout`, `onLoginClick`, `onJoinClick`
    *   `- [ ] NavButton: `iconName`, `label`, `isActive`, `onClick`
    *   `- [ ] ListItem: `children`, `onClick` (for crack items)
    *   `- [ ] ModalInputGroup: `children`
    *   `- [ ] ErrorMessage: `message`
*   **Organisms (유기체):**
    *   `- [ ] AppHeader: `title`, `username`, `isLoggedIn`, `onLogout`, `onLoginClick`, `onJoinClick`
    *   `- [ ] ActionSidebar: `activeView`, `onNavClick`
    *   `- [ ] AuthForm: `type` (login/signup), `onSubmit`, `onSwitchAuth`, `errorMessage`
    *   `- [ ] ContentCard: `title`, `children`
    *   `- [ ] LiveFeedContainer: `status` (live/offline), `message`
    *   `- [ ] CrackAnalysisSummary: `crackData` (array of crack objects), `onCrackItemClick`
    *   `- [ ] StructureManagementList: `structures` (array of structure objects), `onCreateStructureClick`
    *   `- [ ] DetailModal: `isOpen`, `onClose`, `crackDetailData`
    *   `- [ ] CreateStructureModal: `isOpen`, `onClose`, `onSubmit`, `onGenerateLink`

#### **1.3. 글로벌 레이아웃 (Global Layouts)**

*   `- [ ] AppContainer: 전체 애플리케이션을 감싸는 컨테이너`
*   `- [ ] MainWrapper: 헤더와 메인 콘텐츠 영역을 포함하는 래퍼`
*   `- [ ] MainContent: 대시보드의 주요 콘텐츠 영역`
*   `- [ ] ContentView: 각 대시보드 뷰의 기본 레이아웃`
*   `- [ ] AuthLayout: 로그인/회원가입 페이지의 레이아웃`
...
*   `- [ ] `src/` 디렉토리 생성`
*   `- [ ] `assets/` 디렉토리 및 하위 폴더 생성`
*   `- [ ] `components/` 디렉토리 및 `atoms`, `molecules`, `organisms` 하위 폴더 생성`
*   `- [ ] `features/` 디렉토리 및 `auth`, `dashboard`, `common` 하위 폴더 생성 (각각 `components`, `hooks`, `api`, `types` 포함)`
*   `- [ ] `hooks/` 디렉토리 생성`
*   `- [ ] `layouts/` 디렉토리 생성`
*   `- [ ] `pages/` 디렉토리 생성`
*   `- [ ] `services/` 디렉토리 생성`
...
*   `- [ ] Button 컴포넌트 개발 (props: `variant`, `size`, `onClick`, `disabled`, `children`; states: default, hover, active, disabled)`
*   `- [ ] Input 컴포넌트 개발 (props: `type`, `placeholder`, `value`, `onChange`, `readOnly`, `required`; states: default, focused, error, readOnly)`
*   `- [ ] Label 컴포넌트 개발 (props: `htmlFor`, `children`)`
*   `- [ ] Icon 컴포넌트 개발 (props: `name`, `size`, `color`)`
*   `- [ ] Text 컴포넌트 개발 (props: `variant`, `color`, `fontSize`, `fontWeight`, `children`)`
*   `- [ ] Badge 컴포넌트 개발 (props: `variant`, `children`)`

#### **3.2. Molecules (분자) 개발**

*   `- [ ] UserProfileDisplay 컴포넌트 개발 (props: `username`, `isLoggedIn`, `onLogout`, `onLoginClick`, `onJoinClick`)`
*   `- [ ] NavButton 컴포넌트 개발 (props: `iconName`, `label`, `isActive`, `onClick`)`
*   `- [ ] ListItem 컴포넌트 개발 (props: `children`, `onClick`)`
*   `- [ ] ModalInputGroup 컴포넌트 개발 (props: `children`)`
*   `- [ ] ErrorMessage 컴포넌트 개발 (props: `message`)`

#### **3.3. Organisms (유기체) 개발**

*   `- [ ] AppHeader 컴포넌트 개발 (props: `title`, `username`, `isLoggedIn`, `onLogout`, `onLoginClick`, `onJoinClick`)`
*   `- [ ] ActionSidebar 컴포넌트 개발 (props: `activeView`, `onNavClick`)`
*   `- [ ] AuthForm 컴포넌트 개발 (props: `type`, `onSubmit`, `onSwitchAuth`, `errorMessage`)`
*   `- [ ] ContentCard 컴포넌트 개발 (props: `title`, `children`)`
*   `- [ ] LiveFeedContainer 컴포넌트 개발 (props: `status`, `message`)`
*   `- [ ] CrackAnalysisSummary 컴포넌트 개발 (props: `crackData`, `onCrackItemClick`)`
*   `- [ ] StructureManagementList 컴포넌트 개발 (props: `structures`, `onCreateStructureClick`)`
*   `- [ ] DetailModal 컴포넌트 개발 (props: `isOpen`, `onClose`, `crackDetailData`)`
*   `- [ ] CreateStructureModal 컴포넌트 개발 (props: `isOpen`, `onClose`, `onSubmit`, `onGenerateLink`)`
...
*   `- [ ] **서버 상태 (Server State):** `React Query`를 사용하여 API 데이터(균열, 구조물 목록 등) 관리`
*   `- [ ] **전역 클라이언트 상태 (Global Client State):** 사용자 인증 정보 (`isLoggedIn`, `currentUser`), 현재 활성화된 대시보드 뷰 (`activeView`) 등은 `React Context API` 또는 `Zustand`/`Jotai`를 사용하여 관리`
...
*   **실시간 모니터링 (Real-time Monitoring):**
    *   `- [ ] `GET /api/live-feed-status`: 실시간 드론 영상 스트리밍 상태 조회 (response: `{ status: 'live' | 'offline', url?: string }`)`
...
*   **단위 테스트 (Unit Tests):**
    *   `- [ ] Atoms 컴포넌트 (Button, Input 등)의 렌더링 및 상호작용 테스트`
    *   `- [ ] 유틸리티 함수 및 커스텀 훅 로직 테스트`
*   **통합 테스트 (Integration Tests):**
    *   `- [ ] AuthForm과 인증 로직 연동 테스트`
    *   `- [ ] AppHeader 및 ActionSidebar의 뷰 전환 기능 테스트`
    *   `- [ ] 모달 컴포넌트의 열림/닫힘 및 데이터 표시 테스트`
...
*   `- [ ] 데이터 페칭 최적화 (React Query의 캐싱 및 stale-while-revalidate 전략 활용)`
...
*   `- [ ] 크로스 브라우저 호환성 테스트 및 Babel/PostCSS 설정`